...
export const metadata = {
  title: "Services | styleeditbyreena",
  description:
...
export default async function ServicesPage() {
  const [services, content] = await Promise.all([getProducts(), getPageContent("shop")]);
    ...
    <main className="bg-[#fff7f1] text-[#5b1725]">
      <section className="relative overflow-hidden px-4 py-20 text-white sm:px-8 sm:py-24 lg:px-14 lg:py-32">
        ...
        <div className="relative z-10 max-w-4xl">
          <p className="mb-3 text-xs font-black uppercase tracking-normal text-[#f2cbd1]">{content.heroEyebrow}</p>
      ...
      <section className="grid gap-4 px-4 py-16 sm:grid-cols-2 sm:px-8 sm:py-20 lg:grid-cols-3 xl:grid-cols-4 lg:px-14">
        {services.map((item) => (
          <Link className="block overflow-hidden rounded-lg border border-[#d8c1b4] bg-white transition hover:-translate-y-1 hover:shadow-[0_16px_40px_rgba(43,35,32,0.12)]" href={`/services/${item.slug}`} key={item.slug}>

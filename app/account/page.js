    ...
    <main className="bg-[#fff7f1] px-4 py-16 text-[#5b1725] sm:px-8 sm:py-20 lg:px-14 lg:py-28">
      ...
      {orders.length ? (
        <div className="grid gap-4">
          {orders.map((order) => (
            <article className="rounded-lg border border-[#d8c1b4] bg-white p-5" key={order.id}>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-xs font-black uppercase tracking-normal text-[#9f5f6f]">Order #{order.id}</p>
                ...
                <div className="text-left sm:text-right">
                  ...
                  <p className="text-sm text-[#6f5d55]">{formatDate(order.createdAt)}</p>
                </div>
              </div>
            </article>
          ))}
        ...
        <section className="rounded-lg border border-[#d8c1b4] bg-white p-6">
